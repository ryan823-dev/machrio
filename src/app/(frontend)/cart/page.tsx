  ...
  if (items.length === 0) {
    return (
      <div className="container-main py-16 text-center">
        ...
        <h1 className="mt-4 text-2xl font-bold text-secondary-900">{t.cart.emptyTitle}</h1>
        <p className="mt-2 text-secondary-500">{t.cart.emptyText}</p>
  ...
  return (
    <div className="container-main py-8">
      {showCancelledBanner && (
        <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-amber-800">{t.cart.cancelled}</span>
          </div>
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-secondary-900">{t.cart.shoppingCart} ({items.length} {t.cart.items})</h1>
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex items-center gap-3 text-sm text-secondary-600">
            ...
            <span>{selectedItems.size} of {items.length} {t.cart.selected}</span>
          </div>
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Image */}
                <Link
                  href={withLocalePath(`/product/${item.categorySlug}/${item.slug}`, locale)}
        ...
        <div className="lg:col-span-1">
          <div className="rounded-lg border border-secondary-200 bg-white p-6 sticky top-24 space-y-5">
            <h2 className="text-lg font-bold text-secondary-900">{t.cart.orderSummary}</h2>

            {/* Shipping country selector */}
            <div>
              <label className="block text-xs font-medium text-secondary-600">{t.cart.shipTo}</label>
              <select
            ...
            {estimatedShipDate && !noneSelected && (
              <p className="text-xs text-secondary-500">
                {t.cart.shipsOn} <span className="font-medium text-secondary-700">{estimatedShipDate}</span>
              </p>
            )}
            ...
            <Link
              href={canProceedToCheckout ? withLocalePath('/checkout', locale) : '#'}
              className={`btn-primary w-full text-center block ${
                canProceedToCheckout ? '' : 'opacity-50 pointer-events-none'
              }`}
            >
              {canProceedToCheckout ? t.cart.checkout : t.cart.quoteRequired}
            </Link>
            <Link href={withLocalePath('/category', locale)} className="block text-center text-sm text-secondary-500 hover:text-primary-700">
              {t.cart.continueShopping}
            </Link>
          </div>
        </div>
