...
/**
 * 产品数据导入脚本 - 保留原始 slug
 * 从 Excel 文件导入产品到 Supabase，使用 MongoDB 的原始 slug
 */
...
async function main() {
  console.log('=== Machrio 产品导入脚本（保留原始 slug）===\n')
  ...
  console.log(`1. 加载原始 slug 映射: ${slugBySku.size} 个`)
  ...
  try {
    ...
    // 4. 统计匹配情况
    let withOriginalSlug = 0
    let withoutOriginalSlug = 0
    for (const p of products) {
      if (slugBySku.has(p['SKU'])) {
        withOriginalSlug++
      } else {
        withoutOriginalSlug++
      }
    }
    console.log(`   有原始 slug: ${withOriginalSlug}, 无原始 slug: ${withoutOriginalSlug}`)

    // 5. 清空 products 表
    console.log('4. 清空现有产品数据...')
    await pool.query('TRUNCATE TABLE products RESTART IDENTITY CASCADE')
    console.log('   已清空')

    // 6. 导入产品
    console.log('5. 导入产品（使用原始 slug）...')
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      // 使用原始 slug 或生成新的
      const originalSlug = slugBySku.get(p['SKU'])
      const slug = originalSlug || generateSlug(p['Name'], p['SKU'])
    ...
    // 检查 Surface Protection Tape 的产品 slug
    const surfaceResult = await pool.query(`
      SELECT p.sku, p.slug
      FROM products p
      JOIN categories c ON p.primary_category_id = c.id::uuid
      WHERE c.name = 'Surface Protection Tape'
      LIMIT 5
    `)
    console.log('\nSurface Protection Tape 示例 slug:')
    for (const row of surfaceResult.rows) {
      console.log(`  ${row.sku}: ${row.slug}`)
    }
