...
async function ultimateMigrate() {
  console.log('🚀 批量迁移产品分类...\n')
  ...
  try {
    ...
    }, { projection: { slug: 1, primaryCategory: 1 } }).toArray()
    ...
    for (let i = 0; i < updates.length; i += BATCH_SIZE) {
      ...
      const params = batch.flatMap(u => [u.slug, u.categoryId])
      
      const result = await client.query(`
        UPDATE products p
        SET primary_category_id = v.category_id::uuid
        FROM (VALUES ${values}) AS v(slug, category_id)
        WHERE p.slug = v.slug
      `, params)
...
ultimateMigrate().catch(console.error)
